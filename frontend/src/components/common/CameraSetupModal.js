 ...
 * 4-step wizard for adding new IP cameras to the system
 */
class CameraSetupModal {
    constructor() {
        ...
        this.formData = {
            ...
            discovery_method: 'manual', // 'auto' or 'manual'
            ip_address: '',
            discovered_cameras: [],
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" id="connection-port" class="form-input" 
                                   value="${this.formData.port}" min="1" max="65535">
                        </div>
                    </div>
    ...
    getStreamListTemplate() {
        ...
        return this.formData.available_streams.map(stream => `
            <div class="stream-item ${stream.id === this.formData.selected_stream?.id ? 'selected' : ''}" 
    ...
    attachStep2Listeners() {
        ...
        document.querySelectorAll('input[name="discovery-method"]').forEach(radio => {
            radio.addEventListener('change', (e) => {
                ...
                this.render(); // Re-render to show appropriate content
            });
        });
    ...
    validateStep3() {
        // Connection test should be successful to proceed
    ...
    getValidationRules() {
        return {
            ...
            ip_address: { required: true, pattern: /^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$/ }
        };
    }
    ...
    removeEventListeners() {
        // Event listeners are automatically removed when modal is destroyed
    }

    // Async methods (would make actual API calls)
    ...
    async loadAvailableStreams() {
        // Simulate loading available streams
    ...
    async saveCamera() {
        try {
            ...
            // Success - hide modal and trigger refresh
            this.hide();
            window.dispatchEvent(new CustomEvent('cameraAdded', { detail: this.formData }));
    ...
    startPreview() {
        ...
        // Implementation would start actual video stream
    }

    stopPreview() {
        this.formData.preview_active = false;
        // Implementation would stop video stream
    }

    takeSnapshot() {
        // Implementation would capture and save snapshot

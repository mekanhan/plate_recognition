 ...
 * Advanced detection results with filtering, search, and management
 */
class DetectionsPage {
    ...
    loadMockData() {
        // Enhanced mock data based on prototype6
        this.detections = [
            {
                id: 'det_001',
    ...
    getTemplate() {
        return `
            <div class="page-header">
                ...
                <p class="page-subtitle">License plate detection history and analysis</p>
            </div>
            
            <!-- Detection Action Bar -->
            <div class="detection-action-bar">
                ...
                <button class="btn btn-secondary" id="manual-detection-btn">
                    <i class="fas fa-search"></i>
                    Manual Detection
                </button>
                <button class="btn btn-secondary" id="detection-report-btn">
            ...
            <div class="detection-filters">
                ...
                <div class="filter-row filter-row-1">
                    <div class="filter-group smart-search-group">
                        <label for="smart-search">Smart Search:</label>
                        <input type="text" id="smart-search" class="filter-input smart-search-input" 
                               placeholder="🔍 License plate, vehicle type, color..." value="${this.currentFilters.search}">
                    </div>
                    <div class="filter-group quick-filters-group">
                        ...
                        <div class="quick-filter-row">
                            ...
                            <button class="quick-filter-btn" data-period="today">Today</button>
                            <button class="quick-filter-btn" data-period="yesterday">Yesterday</button>
                        </div>
                    </div>
                    <div class="filter-group date-range-group">
                        <label>Date Range:</label>
                        <div class="date-inputs">
                            <input type="date" id="date-from" class="filter-input date-input" value="${this.currentFilters.dateFrom}">
                            <input type="date" id="date-to" class="filter-input date-input" value="${this.currentFilters.dateTo}">
                        </div>
                    </div>
                    <div class="filter-group">
                        ...
                        <select id="camera-filter" class="filter-select">
                            ...
                            <option value="cam_001">Entrance Gate</option>
                            <option value="cam_002">Parking Lot A</option>
                            <option value="cam_003">Loading Dock</option>
                            <option value="cam_004">Side Entrance</option>
                            <option value="cam_005">Rear Exit</option>
                            <option value="cam_006">Visitor Parking</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="filter-row filter-row-2">
                    <div class="filter-group direction-speed-group">
                        <div class="filter-subgroup">
                            <label>Direction:</label>
                            <select id="direction-filter" class="filter-select compact">
                        ...
                        <div class="filter-subgroup speed-range-subgroup">
                            <label>Speed (mph):</label>
                            <div class="range-inputs">
                                <input type="number" id="speed-min" class="filter-input range-input" placeholder="Min" min="0" max="100" value="${this.currentFilters.speedMin}">
                                <span class="range-separator">-</span>
                                <input type="number" id="speed-max" class="filter-input range-input" placeholder="Max" min="0" max="100" value="${this.currentFilters.speedMax}">
                            </div>
                        </div>
                    </div>
                    <div class="filter-group quick-filters-group">
                        ...
                        <div class="quick-filter-row">
                            <button class="quick-filter-btn" data-period="this-week">This Week</button>
                            <button class="quick-filter-btn" data-period="last-week">Last Week</button>
                            <button class="quick-filter-btn" data-period="30d">Last 30 Days</button>
                        </div>
                    </div>
                    <div class="filter-group confidence-status-group">
                        <div class="filter-subgroup">
                            <label>Confidence:</label>
                            <select id="confidence-filter" class="filter-select compact">
                        ...
                        <div class="filter-subgroup">
                            <label>Status:</label>
                            <select id="status-filter" class="filter-select compact">
                    ...
                    <div class="filter-actions">
                        ...
                        <button class="btn btn-secondary" id="clear-all-filters-btn">Clear All</button>
                    </div>
                </div>
            </div>
            ...
            <div class="detection-results-container">
                <div class="detection-table-header">
                    <h3>Detection Results (${this.filteredDetections.length})</h3>
                    <div class="bulk-actions" id="bulk-actions" style="display: none;">
                        <button class="btn btn-small btn-secondary" id="bulk-verify">Verify Selected</button>
                        <button class="btn btn-small btn-warning" id="bulk-flag">Flag Selected</button>
                        <button class="btn btn-small btn-danger" id="bulk-delete">Delete Selected</button>
                    </div>
                </div>
                <div class="detection-table-wrapper">
                    <table class="detection-table" id="detection-table">
                        ...
                        <tbody id="detection-table-body">
                            <!-- Detection rows will be dynamically loaded -->
                        </tbody>
                    </table>
                </div>
                ...
                <div class="modal-container detection-details-container">
                    <div class="modal-header">
                        <div class="modal-title">
                            <i class="fas fa-search"></i>
                            <span>Detection Details - <span id="modal-plate-number">ABC-123</span></span>
                        </div>
                        <button class="modal-close" id="close-detection-modal">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>
                    
                    <div class="modal-content">
                        <div class="detection-details-layout">
                            <!-- Left side - Vehicle Image -->
                            <div class="detection-image-section">
                                <div class="vehicle-image-container">
                                    <div class="vehicle-image" id="modal-vehicle-image">
                                        <i class="fas fa-car"></i>
                                        <div class="plate-overlay">
                                            <span class="plate-text" id="modal-plate-overlay">ABC-123</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="image-actions">
                                    <button class="btn btn-small btn-secondary" id="enhanced-image-btn">
                                        <i class="fas fa-magic"></i>
                                        Enhanced Image
                                    </button>
                                    <button class="btn btn-small btn-secondary" id="video-clip-btn">
                            ...
                            <div class="detection-info-section">
                                <div class="detection-tabs">
                                    <button class="detection-tab active" data-tab="basic">Basic Info</button>
                                    <button class="detection-tab" data-tab="vehicle">Vehicle Details</button>
                                    <button class="detection-tab" data-tab="confidence">Confidence Analysis</button>
                                </div>
                                
                                <div class="detection-tab-content">
                                    <!-- Basic Info Tab -->
                                    ...
                                    <!-- Vehicle Details Tab -->
                                    <div class="tab-panel" id="vehicle-tab">
                                        <div class="info-grid" id="vehicle-info-grid">
                                            <!-- Dynamic content -->
                                        </div>
                                    </div>
                                    
                                    <!-- Confidence Analysis Tab -->
                                    <div class="tab-panel" id="confidence-tab">
                    ...
                    <div class="modal-footer">
                        <button class="btn btn-warning" id="flag-detection-btn">
                            <i class="fas fa-flag"></i>
                            Flag Detection
                        </button>
                        <button class="btn btn-success" id="download-detection-btn">
    ...
    attachEventListeners() {
        ...
        document.querySelectorAll('.quick-filter-btn').forEach(btn => {
            btn.addEventListener('click', (e) => this.handleQuickFilter(e.target.dataset.period));
        });
        ...
        document.getElementById('direction-filter')?.addEventListener('change', (e) => this.updateFilter('direction', e.target.value));
        ...
        // Speed range filters
        document.getElementById('speed-min')?.addEventListener('input', (e) => this.updateFilter('speedMin', e.target.value));
        document.getElementById('speed-max')?.addEventListener('input', (e) => this.updateFilter('speedMax', e.target.value));
        ...
        // Modal events
        document.getElementById('close-detection-modal')?.addEventListener('click', () => this.closeModal());
    ...
    applyFilters() {
        this.filteredDetections = this.detections.filter(detection => {
            ...
            if (this.currentFilters.search) {
                ...
                const searchable = `${detection.plate_number} ${detection.vehicle_color} ${detection.vehicle_type} ${detection.vehicle_make} ${detection.vehicle_model} ${detection.camera_name}`.toLowerCase();
            ...
            // Direction filter
            if (this.currentFilters.direction && detection.direction !== this.currentFilters.direction) {
                return false;
            }

            // Speed range filter
            if (this.currentFilters.speedMin && detection.speed < parseInt(this.currentFilters.speedMin)) {
                return false;
            }
            if (this.currentFilters.speedMax && detection.speed > parseInt(this.currentFilters.speedMax)) {
                return false;
            }
            ...
            if (this.currentFilters.dateFrom) {
                ...
                if (detection.timestamp < fromDate) return false;
            }
            if (this.currentFilters.dateTo) {
                const toDate = new Date(this.currentFilters.dateTo);
                toDate.setHours(23, 59, 59, 999); // End of day
                if (detection.timestamp > toDate) return false;
            }
    ...
    handleSort(column) {
        if (this.currentSort.column === column) {
            ...
            if (this.currentSort.direction === null) {
        ...
        this.filteredDetections.sort((a, b) => {
            ...
            switch(this.currentSort.column) {
                ...
                case 'confidence':
                    ...
                    break;
                case 'vehicle':
                    aVal = `${a.vehicle_color} ${a.vehicle_type}`.toLowerCase();
                    bVal = `${b.vehicle_color} ${b.vehicle_type}`.toLowerCase();
                    break;
                case 'status':
            ...
            <tr data-detection-id="${detection.id}" ${this.selectedDetections.has(detection.id) ? 'class="selected"' : ''}>
                <td class="checkbox-column">
                    <input type="checkbox" ${this.selectedDetections.has(detection.id) ? 'checked' : ''} 
                           onchange="detectionsPage.handleRowSelect('${detection.id}', this.checked)">
                </td>
                <td>${this.formatTimestamp(detection.timestamp)}</td>
                ...
                <td class="vehicle-info">${detection.vehicle_color} ${detection.vehicle_type}</td>
                <td>
                ...
                <td class="actions-column">
                    <div class="row-actions">
                        <button class="row-action-btn view" onclick="detectionsPage.showDetails('${detection.id}')" title="View Details">
                            <i class="fas fa-eye"></i>
                        </button>
                        <button class="row-action-btn download" onclick="detectionsPage.downloadDetection('${detection.id}')" title="Download">
                            <i class="fas fa-download"></i>
                        </button>
                        <button class="row-action-btn flag" onclick="detectionsPage.toggleFlag('${detection.id}')" title="Flag/Unflag">
                            <i class="fas fa-flag"></i>
                        </button>
                    </div>
                </td>
    ...
    updateResultCount() {
        const header = document.querySelector('.detection-table-header h3');
        if (header) {
            header.textContent = `Detection Results (${this.filteredDetections.length})`;
        }
    }
    ...
    populateModal(detection) {
        ...
        basicInfoGrid.innerHTML = `
            ...
            <div class="info-item">
                <label>Status:</label>
                <span class="status-badge ${detection.status}">${this.capitalizeFirst(detection.status)}</span>
            </div>
            <div class="info-item">
                <label>Processing Time:</label>
                <span>${detection.processing_time}ms</span>
            </div>
        `;

        // Populate vehicle info tab
        const vehicleInfoGrid = document.getElementById('vehicle-info-grid');
        vehicleInfoGrid.innerHTML = `
            <div class="info-item">
            ...
            <div class="info-item">
                ...
                <span>${detection.vehicle_make} ${detection.vehicle_model}</span>
            </div>
            <div class="info-item">
                <label>Direction:</label>
                <span>${this.capitalizeFirst(detection.direction)}</span>
            </div>
            <div class="info-item">
                <label>Speed:</label>
                <span>${detection.speed} mph</span>
            </div>
        `;

        // Populate confidence analysis
        ...
        confidenceBreakdown.innerHTML = `
            <div class="confidence-item">
                ...
                <div class="confidence-bar">
                    ...
                    <span class="confidence-percent">${detection.confidence.toFixed(1)}%</span>
                </div>
            </div>
            <div class="confidence-item">
                <label>Character Recognition:</label>
                <div class="confidence-bar">
                    <div class="confidence-fill" style="width: ${detection.confidence - 1.3}%"></div>
                    <span class="confidence-percent">${(detection.confidence - 1.3).toFixed(1)}%</span>
                </div>
            </div>
            <div class="confidence-item">
                <label>Plate Detection:</label>
                <div class="confidence-bar">
                    <div class="confidence-fill" style="width: ${Math.min(detection.confidence + 1.3, 99.8)}%"></div>
                    <span class="confidence-percent">${Math.min(detection.confidence + 1.3, 99.8).toFixed(1)}%</span>
                </div>
            </div>
        `;
    }
    ...
    // Export methods
    exportCSV() {
        const detections = this.selectedDetections.size > 0 
            ? this.filteredDetections.filter(d => this.selectedDetections.has(d.id))
            : this.filteredDetections;

        const csvContent = this.generateCSV(detections);
        this.downloadFile(csvContent, 'detections.csv', 'text/csv');
        this.showToast(`CSV export completed. ${detections.length} detections exported.`, 'success');
    }
    ...
    generateCSV(detections) {
        const headers = ['Timestamp', 'Plate Number', 'Camera', 'Vehicle', 'Confidence', 'Status', 'Direction', 'Speed'];
        const rows = detections.map(d => [
            this.formatTimestamp(d.timestamp),
            d.plate_number,
            d.camera_name,
            `${d.vehicle_color} ${d.vehicle_type} ${d.vehicle_make} ${d.vehicle_model}`,
    ...
    downloadDetection(detectionId) {
        const detection = this.detections.find(d => d.id === detectionId);
        if (detection) {
            const jsonContent = JSON.stringify(detection, null, 2);
            this.downloadFile(jsonContent, `detection_${detection.plate_number}.json`, 'application/json');
            this.showToast(`Detection ${detection.plate_number} downloaded.`, 'success');
        }
    }

    toggleFlag(detectionId) {
        const detection = this.detections.find(d => d.id === detectionId);
        if (detection) {

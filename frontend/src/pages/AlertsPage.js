 ...
 * System alerts, notifications, and alert management
 */
class AlertsPage {
    constructor() {
        this.alerts = [];
    ...
    renderAlerts() {
        ...
        container.innerHTML = this.filteredAlerts.map(alert => 
            this.currentView === 'list' ? this.renderAlertRow(alert) : this.renderAlertCard(alert)
        ).join('');
    ...
    renderAlertRow(alert) {
        ...
        return `
            <div class="alert-row ${alert.severity} ${alert.status}" data-alert-id="${alert.id}">
                <div class="alert-checkbox" style="display: ${this.bulkMode ? 'block' : 'none'}">
                    <input type="checkbox" class="alert-select" data-alert-id="${alert.id}" ${this.selectedAlerts.has(alert.id) ? 'checked' : ''}>
                </div>
                ...
                <div class="alert-content">
                    <div class="alert-header">
                        ...
                        <div class="alert-badges">
                            <span class="alert-badge status ${alert.status}">${this.capitalizeFirst(alert.status)}</span>
                            <span class="alert-badge category">${this.capitalizeFirst(alert.category)}</span>
                        </div>
                    </div>
                ...
                <div class="alert-actions">
                    ${!alert.acknowledged ? `
                    <button class="action-btn acknowledge" data-action="acknowledge" data-alert-id="${alert.id}" title="Acknowledge">
                        <i class="fas fa-check"></i>
                    </button>
                    ` : ''}
                    ${alert.status !== 'resolved' ? `
                    <button class="action-btn resolve" data-action="resolve" data-alert-id="${alert.id}" title="Resolve">
                        <i class="fas fa-check-double"></i>
                    </button>
                    ` : ''}
                    <button class="action-btn details" data-action="details" data-alert-id="${alert.id}" title="View Details">
                        <i class="fas fa-info-circle"></i>
                    </button>
                    <button class="action-btn delete danger" data-action="delete" data-alert-id="${alert.id}" title="Delete">
    ...
    renderAlertCard(alert) {
        ...
        return `
            <div class="alert-card ${alert.severity} ${alert.status}" data-alert-id="${alert.id}">
                <div class="alert-card-header">
                    ...
                    <div class="alert-card-checkbox" style="display: ${this.bulkMode ? 'block' : 'none'}">
                        <input type="checkbox" class="alert-select" data-alert-id="${alert.id}" ${this.selectedAlerts.has(alert.id) ? 'checked' : ''}>
                    </div>
                </div>
                ...
                <div class="alert-card-body">
                    ...
                    <div class="alert-card-meta">
                        ...
                        ${alert.acknowledged ? `
                        <div class="meta-item acknowledged">
                            <i class="fas fa-user-check"></i>
                ...
                <div class="alert-card-actions">
                    ${!alert.acknowledged ? `
                    <button class="btn btn-small btn-success" data-action="acknowledge" data-alert-id="${alert.id}">
                        <i class="fas fa-check"></i>
                        Acknowledge
                    </button>
                    ` : ''}
                    ${alert.status !== 'resolved' ? `
                    <button class="btn btn-small btn-primary" data-action="resolve" data-alert-id="${alert.id}">
                        <i class="fas fa-check-double"></i>
                        Resolve
                    </button>
                    ` : ''}
                    <button class="btn btn-small btn-secondary" data-action="details" data-alert-id="${alert.id}">
                        <i class="fas fa-info-circle"></i>
                        Details
                    </button>
                    <button class="btn btn-small btn-danger" data-action="delete" data-alert-id="${alert.id}">
                        <i class="fas fa-trash"></i>
                        Delete
                    </button>
                </div>
    ...
    attachEventListeners() {
        // Quick actions
        document.getElementById('acknowledge-all-btn')?.addEventListener('click', () => this.acknowledgeAll());
        ...
        document.getElementById('refresh-alerts-btn')?.addEventListener('click', () => this.refreshAlerts());
        ...
        // Filters
        document.getElementById('apply-alert-filters')?.addEventListener('click', () => this.applyFilters());
        document.getElementById('clear-alert-filters')?.addEventListener('click', () => this.clearFilters());
        document.getElementById('alert-search-filter')?.addEventListener('input', (e) => this.handleSearchInput(e));

        // Bulk actions
        document.getElementById('bulk-acknowledge')?.addEventListener('click', () => this.bulkAction('acknowledge'));
    ...
    attachAlertEventListeners() {
        // Alert selection checkboxes
        document.querySelectorAll('.alert-select').forEach(checkbox => {
            checkbox.addEventListener('change', (e) => this.handleAlertSelection(e));
        });

        // Alert action buttons
        document.querySelectorAll('.action-btn[data-action], .btn[data-action]').forEach(btn => {
    ...
    updateBulkActionsBar() {
        ...
        const selectedCount = document.getElementById('selected-alert-count');
    ...
    handleAlertAction(e) {
        ...
        const alertId = e.target.dataset.alertId || e.target.parentElement.dataset.alertId;
    ...
    async resolveAlert(alertId) {
        ...
        try {
            ...
            this.renderAlerts();
            this.render(); // Re-render to update summary cards
    ...
    async deleteAlert(alertId) {
        ...
        if (confirm(`Are you sure you want to delete the alert "${alert.title}"?`)) {
            try {
    ...
    async acknowledgeAll() {
        const unacknowledgedAlerts = this.alerts.filter(a => !a.acknowledged && a.status !== 'resolved');
        
        if (unacknowledgedAlerts.length === 0) {
            this.showToast('No alerts to acknowledge', 'info');
            return;
        }

        if (confirm(`Are you sure you want to acknowledge all ${unacknowledgedAlerts.length} active alerts?`)) {
            try {
                ...
                this.renderAlerts();
                this.showToast(`${unacknowledgedAlerts.length} alerts acknowledged`, 'success');
            } catch (error) {
    ...
    applyFilters() {
        this.filters.severity = document.getElementById('alert-severity-filter').value;
        this.filters.status = document.getElementById('alert-status-filter').value;
        this.filters.category = document.getElementById('alert-category-filter').value;
        this.filters.search = document.getElementById('alert-search-filter').value.toLowerCase();
        ...
        this.filteredAlerts = this.alerts.filter(alert => {
            ...
            const matchesCategory = !this.filters.category || alert.category === this.filters.category;
            const matchesSearch = !this.filters.search || 
                alert.title.toLowerCase().includes(this.filters.search) ||
                alert.message.toLowerCase().includes(this.filters.search) ||
                alert.source.toLowerCase().includes(this.filters.search);
    ...
    clearFilters() {
        ...
        document.getElementById('alert-search-filter').value = '';
        
        this.filters = { severity: '', status: '', category: '', search: '' };
        this.filteredAlerts = [...this.alerts];
        this.renderAlerts();
    }
    ...
    toggleBulkMode() {
        ...
        const checkboxes = document.querySelectorAll('.alert-checkbox, .alert-card-checkbox');
    ...
    closeBulkMode() {
        ...
        this.selectedAlerts.clear();
        document.querySelectorAll('.alert-select').forEach(cb => cb.checked = false);
        document.querySelectorAll('.alert-checkbox, .alert-card-checkbox').forEach(cb => cb.style.display = 'none');
        this.updateBulkActionsBar();
    }

    async bulkAction(action) {
        const selectedIds = Array.from(this.selectedAlerts);
        if (selectedIds.length === 0) return;

        const confirmMessage = `Are you sure you want to ${action} ${selectedIds.length} alert(s)?`;
    ...
    exportAlerts() {
        ...
        this.downloadFile(csvContent, `alerts_export_${new Date().toISOString().split('T')[0]}.csv`, 'text/csv');
        this.showToast('Alerts exported successfully', 'success');
    }

    generateAlertsCSV() {
        const headers = ['ID', 'Title', 'Message', 'Severity', 'Status', 'Category', 'Source', 'Timestamp', 'Acknowledged', 'Acknowledged By', 'Resolved At'];
    ...
    async refreshAlerts() {
        ...
        try {
            ...
            this.render(); // Re-render to update summary cards
            this.showToast('Alerts refreshed successfully', 'success');
        } catch (error) {
